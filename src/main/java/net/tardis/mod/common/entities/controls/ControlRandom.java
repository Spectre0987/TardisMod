	...
	public void preformAction(EntityPlayer player) {
		if (!world.isRemote) {
			...
			if (maxDist == 0) {
				...
				tardis.setDesination(new BlockPos((loc.getX() + rand.nextInt(rand.nextInt(maxDist))) - rand.nextInt(maxDist) / 2, 64, (loc.getZ() + rand.nextInt(maxDist)) - rand.nextInt(maxDist) / 2), tardis.getTargetDim());
		} else
			this.ticks = 20;
	}
