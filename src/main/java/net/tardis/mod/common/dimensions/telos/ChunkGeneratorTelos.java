	...
	public Chunk generateChunk(int cx, int cz) {
		...
		for(int x = 0; x < 16; ++x) {
			for(int z = 0; z < 16; ++z) {
				...
				double nv = noise.getValue(x, z);
				int height = noise.getValue(x, z) > 0 ? 1 : 0;
				c.setBlockState(new BlockPos(x, c.getHeightValue(x, z) + height, z), Blocks.BRICK_BLOCK.getDefaultState());
			}
		}
