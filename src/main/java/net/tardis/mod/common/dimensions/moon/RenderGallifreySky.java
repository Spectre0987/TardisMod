...
public class RenderGallifreySky extends IRenderHandler {
	...
	public static RenderGallifreySky getInstance() {
		if (INSTANCE == null)
			INSTANCE = new RenderGallifreySky();
		return INSTANCE;
	}
	...
	public void render(float partialTicks, WorldClient world, Minecraft mc) {
		...
		boolean isDay = world.getWorldTime() <= 13000;
		...
		boolean pass = mc.gameSettings.anaglyph;
		...
		GlStateManager.color(1, 1, 1, 1F - opacity);
		bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX);
		bufferbuilder.pos((double) (-f17), 100.0D, (double) (-f17)).tex(0.0D, 0.0D).endVertex();
		bufferbuilder.pos((double) f17, 100.0D, (double) (-f17)).tex(1.0D, 0.0D).endVertex();
		bufferbuilder.pos((double) f17, 100.0D, (double) f17).tex(1.0D, 1.0D).endVertex();
