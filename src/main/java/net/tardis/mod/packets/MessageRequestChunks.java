			...
			ctx.getServerHandler().player.getServerWorld().addScheduledTask(() -> {
				for (int x = message.chunkX - message.radius; x < message.chunkX + message.radius; x++)
					for (int z = message.chunkZ - message.radius; z < message.chunkZ + message.radius; z++)
						Tardis.NETWORK.sendToAll(new MessageChunkData(ctx.getServerHandler().player.world.getMinecraftServer().getWorld(message.dimensionID).getChunkProvider().provideChunk(x, z), 65535, message.dimensionID));
