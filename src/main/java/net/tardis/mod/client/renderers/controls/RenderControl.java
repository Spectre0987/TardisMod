...
public abstract class RenderControl extends Render<EntityControl> {
    ...
    public static void renderText(EntityControl entityControl, String text) {
        GlStateManager.pushMatrix();
        GlStateManager.color(0, 0, 0, 0.4f);
        FontRenderer fontrenderer = Minecraft.getMinecraft().fontRenderer;
        GlStateManager.translate(0, 1.1 - entityControl.getEntityBoundingBox().maxY, 0);

        float offset = MathHelper.cos(entityControl.ticksExisted * 0.1F) * -0.09F;
        ...
        fontrenderer.drawStringWithShadow(text, -fontrenderer.getStringWidth(text) / 2, (int) -offset, 553648127);
        GL11.glPopMatrix();
        ...
        GL11.glPushMatrix();
        GL11.glScalef(0.3F, 0.3F, 0.3F);
        fontrenderer.drawStringWithShadow(text, -fontrenderer.getStringWidth(text) / 2, (int) -offset, -1);
        GL11.glPopMatrix();
    ...
    public void doRender(EntityControl entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        if (mc.player.getHeldItemMainhand().getItem() == TItems.manual) {
            ...
            if (look != null && entity != null && look.getEntityId() == entity.getEntityId()) {
                renderText(entity, entity.getDisplayName().getUnformattedText());
            }
        }
    }
